...
// Shims for some Arrow utilities used in the actual PR
class ParquetException : public std::runtime_error {
 public:
  ParquetException(std::string msg) : std::runtime_error(msg) {}
  ParquetException(std::string msg, uint32_t stuff)
      : std::runtime_error(msg + std::to_string(stuff)) {}
  ParquetException(std::string msg, uint32_t stuff, std::string msg2)
      : std::runtime_error(msg + std::to_string(stuff) + msg2) {}
  ParquetException(std::string msg, uint32_t stuff, std::string msg2, uint32_t stuff2,
                   std::string msg3)
      : std::runtime_error(msg + std::to_string(stuff) + msg2 + std::to_string(stuff2) +
                           msg3) {}

  static ParquetException NYI() { return ParquetException("not yet implemented"); }
};
