...
TEST(GeoArrowHppTest, ParseSwappedEndian) {
  ...
  std::vector<uint8_t> linestring_be(
...
TEST(GeoArrowHppTest, InvalidWKBArray) {
  std::vector<uint8_t> valid_polygon_with_extra_byte(
  ...
  std::vector<uint8_t> bad_geometry_type({0x01, 0x01, 0x01, 0x01, 0x01});
  std::vector<uint8_t> bad_endian({0xff, 0x01, 0x01, 0x01, 0x01});
