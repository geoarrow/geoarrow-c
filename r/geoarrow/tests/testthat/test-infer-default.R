...
test_that("geoarrow_infer_schema() works for mixed vectors", {
  vec <- wk::wkt(c("POINT (0 1)", "LINESTRING (2 3, 4 5)"), crs = "OGC:CRS84")

  schema <- geoarrow_infer_schema(vec)
...
test_that("geoarrow_infer_schema() works for single-type vectors", {
...
test_that("geoarrow_infer_schema() respects coord_type", {
  vec <- wk::wkt(c("POINT (0 1)", "POINT (2 3)"))

  schema <- geoarrow_infer_schema(vec, coord_type = "INTERLEAVED")
  parsed <- geoarrow_schema_parse(schema)
  expect_identical(parsed$geometry_type, enum$GeometryType$POINT)
  expect_identical(parsed$dimensions, enum$Dimensions$XY)
...
test_that("geoarrow_infer_schema() can promote mixed points to multi", {
...
test_that("geoarrow_infer_schema() can promote mixed linestrings to multi", {
...
test_that("geoarrow_infer_schema() can promote mixed polygons to multi", {
  vec <- wk::wkt(
...
test_that("geoarrow_infer_schema() works for mixed dimensions (Z)", {
...
test_that("geoarrow_infer_schema() works for mixed dimensions (M)", {
...
test_that("geoarrow_infer_schema() works for mixed dimensions (ZM)", {
