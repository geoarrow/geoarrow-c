...
test_that("geoarrow_infer_schema_default() works for mixed vectors", {
  vec <- wk::wkt(c("POINT (0 1)", "LINESTRING (2 3, 4 5)"), crs = "OGC:CRS84")

  schema <- geoarrow_infer_schema_default(vec)
...
test_that("geoarrow_infer_schema_default() works for single-type vectors", {
...
test_that("geoarrow_infer_schema_default() can promote mixed linestrings to multi", {
...
test_that("geoarrow_infer_schema_default() can promote mixed polygons to multi", {
  vec <- wk::wkt(
...
test_that("geoarrow_infer_schema_default() works for mixed dimensions (Z)", {
...
test_that("geoarrow_infer_schema_default() works for mixed dimensions (M)", {
...
test_that("geoarrow_infer_schema_default() works for mixed dimensions (ZM)", {
